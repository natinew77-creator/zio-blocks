  ...
  def fromImpl[A <: AnyKind : Type](using Quotes): Expr[TypeId[A]] = {
        ...
        tpe.dealias match {
          ...
          case AppliedType(base, args) =>
             base.dealias match {
               ...
               case _ => getTypeIdImpl(base) // Fallback, might lose args if structure is weird
             }
    ...
    def makeTypeId(sym: Symbol, args: List[TypeRepr], deep: Boolean): Expr[TypeId[?]] = {
      ...
      val parentsExpr = if (deep && !sym.isNoSymbol) {
         val relevantParents = sym.typeRef.baseClasses.filter(_ != sym).filter(_.name != "Object").filter(_.name != "Any")
         Expr.ofList(relevantParents.map(s => makeTypeRepr(s.typeRef).asInstanceOf[Expr[zio.blocks.typeid.TypeRepr]])).asInstanceOf[Expr[List[zio.blocks.typeid.TypeRepr]]]
      } else {
         Expr.ofList(Nil).asInstanceOf[Expr[List[zio.blocks.typeid.TypeRepr]]]
      }
      
      val argsExpr = Expr.ofList(args.map(t => makeTypeRepr(t).asInstanceOf[Expr[zio.blocks.typeid.TypeRepr]])).asInstanceOf[Expr[List[zio.blocks.typeid.TypeRepr]]]
    ...
    def makeSyntheticTypeId(name: String, parents: List[TypeRepr]): Expr[TypeId[?]] = {
       ...
       val parentsExpr = Expr.ofList(sorted.map(p => makeTypeRepr(p).asInstanceOf[Expr[zio.blocks.typeid.TypeRepr]])).asInstanceOf[Expr[List[zio.blocks.typeid.TypeRepr]]]
       '{
         TypeId(
           Owner.Root,
           ${Expr(name)},
