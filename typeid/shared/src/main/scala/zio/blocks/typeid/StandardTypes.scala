...
object StandardTypes {
  ...
  // Big numbers
  val bigInt: TypeId[BigInt]         = TypeId(scalaOwner, "math.BigInt", Nil, Class(), Nil) // simplified name, actually scala.math.BigInt
  ...
  val localDateTime: TypeId[java.time.LocalDateTime]   = TypeId(javaTime, "LocalDateTime", Nil, Class(isFinal = true), Nil)
  ...
  val offsetDateTime: TypeId[java.time.OffsetDateTime] = TypeId(javaTime, "OffsetDateTime", Nil, Class(isFinal = true), Nil)
  ...
  val zonedDateTime: TypeId[java.time.ZonedDateTime]   = TypeId(javaTime, "ZonedDateTime", Nil, Class(isFinal = true), Nil)
  ...
  val ListId: TypeId[List[_]] = TypeId(scalaColImm, "List", List(tparam("A", 0, Variance.Covariant)), Trait(isSealed = true), Nil)
  val VectorId: TypeId[Vector[_]] = TypeId(scalaColImm, "Vector", List(tparam("A", 0, Variance.Covariant)), Class(isFinal = true), Nil)
  val SetId: TypeId[Set[_]] = TypeId(scalaColImm, "Set", List(tparam("A", 0, Variance.Covariant)), Trait(), Nil)
  val MapId: TypeId[Map[_, _]] = TypeId(scalaColImm, "Map", List(tparam("K", 0, Variance.Invariant), tparam("V", 1, Variance.Covariant)), Trait(), Nil)
  val OptionId: TypeId[Option[_]] = TypeId(scalaOwner, "Option", List(tparam("A", 0, Variance.Covariant)), Trait(isSealed = true), Nil)
  val EitherId: TypeId[Either[_, _]] = TypeId(scalaOwner, "Either", List(tparam("A", 0, Variance.Covariant), tparam("B", 1, Variance.Covariant)), Trait(isSealed = true), Nil)
  val IndexedSeqId: TypeId[IndexedSeq[_]] = TypeId(scalaColImm, "IndexedSeq", List(tparam("A", 0, Variance.Covariant)), Trait(), Nil)
