...
object Reflect {
  ...
  case class Primitive[F[_, _], A](
    ...
    def typeName(value: TypeId[A]): Primitive[F, A] = copy(typeId = value)
  ...
  def unit[F[_, _]](implicit F: FromBinding[F]): Reflect[F, Unit] = primitive(new PrimitiveType.Unit(Validation.None))
  ...
  private[this] def some[F[_, _], A <: AnyRef](element: Reflect[F, A])(implicit F: FromBinding[F]): Record[F, Some[A]] =
    new Record(Vector(new Term("value", element)), TypeId.from[Some[A]], F.fromBinding(Binding.Record.some))
  ...
  object Extractors {
    object List {
      def unapply[F[_, _], A](reflect: Reflect[F, List[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == StandardTypes.ListId =>
    ...
    object Vector {
      def unapply[F[_, _], A](reflect: Reflect[F, Vector[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == StandardTypes.VectorId =>
            x.sequence.element.asInstanceOf[Reflect[F, A]]
        }
    }
    ...
    object Set {
      def unapply[F[_, _], A](reflect: Reflect[F, Set[A]]): Option[Reflect[F, A]] =
        reflect.asSequenceUnknown.collect {
          case x if x.sequence.typeId == StandardTypes.SetId =>
            x.sequence.element.asInstanceOf[Reflect[F, A]]
        }
    }
  }
